...
$sql = "select to_char(a.RECORD_TIME, 'yyyy-mm-dd') RECORD_TIME, a.SITE_NAME SITE_NAME, a.CELL_NAME CELL_NAME, a.trig_type trig_type, c.occur occur30, d.occur occur7, 
...
(select record_time,SITE_NAME,CELL_NAME,TRIG_TYPE from MPT_LTE_TAIL_CELLS_D where record_time >= trunc(sysdate-1) and vendor = 'NSN')
pivot (count(trig_type) for trig_type in ('RRC' as RRC, 'ERAB_STP_SSR' AS ERAB_STP_SSR, 'ERAB_DCR' AS ERAB_DCR)))) a,
-- occur
(select cell_name, count(cell_name) occur from 
(select distinct record_time, cell_name from MPT_LTE_TAIL_CELLS_D where record_time >= trunc(sysdate-30) and vendor = 'NSN') 
group by cell_name) c,
(select cell_name, count(cell_name) occur from 
(select distinct record_time, cell_name from MPT_LTE_TAIL_CELLS_D where record_time >= trunc(sysdate-7) and vendor = 'NSN') 
group by cell_name) d
where record_time >= trunc(sysdate-1)
